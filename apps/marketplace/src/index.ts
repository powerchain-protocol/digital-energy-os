...
export const application = createApplication({
  ...
  routes: [
    { method: "GET", path: "/api/v1/marketplace/health", summary: "Read marketplace service health", handler() { return json({data:{service:"marketplace",version:"1.0.0",persistence:"memory-harness"}}); } },
    ...
    { method: "POST", path: "/api/v1/marketplace/listings", summary: "Create a draft listing", async handler(request) { try { return json(marketplaceService.createListing(await readJson<Omit<MarketplaceListing, "id" | "remaining" | "status" | "createdAt" | "updatedAt">>(request),idempotencyKey(request)), { status: 201 }); } catch (error) { return marketplaceFailure(error); } } },
    ...
    { method: "POST", path: "/api/v1/marketplace/listings/:id/orders", summary: "Reserve inventory", async handler(request, { params }) { const body = await readJson<{ buyerId?: string; quantity?: number }>(request); try { return json(marketplaceService.reserve(params.id, body.buyerId ?? "", body.quantity ?? 0,idempotencyKey(request)), { status: 201 }); } catch (error) { return marketplaceFailure(error); } } },
