...
export function createCetusClient(options: CetusOptions) {
  if (options.fullNodeUrl && !options.fullNodeUrl.startsWith("https://")) {
    throw new PowerChainError("Cetus full node URL must use HTTPS", "VALIDATION_ERROR", 400);
  }
