...
import { Bot, Coins, Gauge, Link as LinkIcon, Upload } from "lucide-react";
...
import { ConversationCreditSummaryCard } from "./conversation-credit-summary";
...
export function ChatInterface() {
  ...
  async function submit(event?: FormEvent) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload?.error?.message ?? payload?.error ?? "Chat failed");
    ...
    <div className="grid min-h-0 gap-5 2xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="panel flex min-h-[min(760px,calc(100dvh-12rem))] min-w-0 flex-col overflow-hidden">
        <header className="flex items-center justify-between gap-4 border-b border-[var(--border)] px-4 py-3.5 sm:px-5">
          ...
          <div className="flex items-center gap-2"><a href="/settings/credits" className="inline-flex h-10 items-center gap-2 rounded-xl border border-[var(--border)] px-3 text-xs font-semibold text-emerald-800 transition hover:bg-emerald-900/[.05]"><Coins className="h-4 w-4"/>Credits</a><AiSettingsDrawer /></div>
        </header>
      ...
      <aside className="grid min-w-0 gap-4 md:grid-cols-2 2xl:block 2xl:space-y-5">
        ...
        <ConversationCreditSummaryCard conversationId={chatId} />
        <div className="panel p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-3"><a href="/settings/integrations" className="inline-flex text-sm font-semibold text-emerald-700 hover:underline">Configure AI models</a><a href="/settings/credits" className="inline-flex text-sm font-semibold text-emerald-700 hover:underline">PWRC / Credits</a></div>
        </div>
        <div className="panel p-4">
