...
export interface OnrampAdapterRequest { operation: string; payload?: Record<string, unknown>; }
export interface OnrampAdapterResponse { provider: "onramp"; operation: string; payload: Record<string, unknown>; }

/** Onramp: fiat on-ramp quote and session creation. Never returns synthetic production data. */
export class OnrampAdapter implements IntegrationAdapter<OnrampAdapterRequest, OnrampAdapterResponse> {
  readonly provider = "onramp";
  constructor(private readonly endpoint?: string, private readonly credentialReference?: string) {}
  async execute(request: OnrampAdapterRequest, context: IntegrationContext): Promise<IntegrationResult<OnrampAdapterResponse>> {
    if (!this.endpoint) return { state: "misconfigured", source: this.provider, observedAt: new Date().toISOString(), error: { code: "INVALID_CONFIGURATION", message: "Onramp endpoint is not configured", retryable: false } };
    ...
    try {
      ...
      if (!response.ok) return { state: response.status === 429 ? "degraded" : "unavailable", source: this.provider, observedAt: new Date().toISOString(), error: { code: response.status === 429 ? "RATE_LIMITED" : "PROVIDER_UNAVAILABLE", message: "Onramp request failed", retryable: response.status >= 429, providerStatus: response.status } };
      const payload = await response.json() as Record<string, unknown>;
      return { state: "available", source: this.provider, observedAt: new Date().toISOString(), data: { provider: "onramp", operation: request.operation, payload } };
    } catch (error) {
