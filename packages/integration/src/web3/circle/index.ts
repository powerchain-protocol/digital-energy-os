...
export interface CircleAdapterRequest { operation: string; payload?: Record<string, unknown>; }
export interface CircleAdapterResponse { provider: "circle"; operation: string; payload: Record<string, unknown>; }

/** Circle: USDC wallet, transfer, and payment operations. Never returns synthetic production data. */
export class CircleAdapter implements IntegrationAdapter<CircleAdapterRequest, CircleAdapterResponse> {
  readonly provider = "circle";
  constructor(private readonly endpoint?: string, private readonly credentialReference?: string) {}
  async execute(request: CircleAdapterRequest, context: IntegrationContext): Promise<IntegrationResult<CircleAdapterResponse>> {
    if (!this.endpoint) return { state: "misconfigured", source: this.provider, observedAt: new Date().toISOString(), error: { code: "INVALID_CONFIGURATION", message: "Circle endpoint is not configured", retryable: false } };
    ...
    try {
      ...
      if (!response.ok) return { state: response.status === 429 ? "degraded" : "unavailable", source: this.provider, observedAt: new Date().toISOString(), error: { code: response.status === 429 ? "RATE_LIMITED" : "PROVIDER_UNAVAILABLE", message: "Circle request failed", retryable: response.status >= 429, providerStatus: response.status } };
      const payload = await response.json() as Record<string, unknown>;
      return { state: "available", source: this.provider, observedAt: new Date().toISOString(), data: { provider: "circle", operation: request.operation, payload } };
    } catch (error) {
