...
export interface OrcaAdapterRequest { operation: string; payload?: Record<string, unknown>; }
export interface OrcaAdapterResponse { provider: "orca"; operation: string; payload: Record<string, unknown>; }

/** Orca: Whirlpool liquidity and swap operations. Never returns synthetic production data. */
export class OrcaAdapter implements IntegrationAdapter<OrcaAdapterRequest, OrcaAdapterResponse> {
  readonly provider = "orca";
  constructor(private readonly endpoint?: string, private readonly credentialReference?: string) {}
  async execute(request: OrcaAdapterRequest, context: IntegrationContext): Promise<IntegrationResult<OrcaAdapterResponse>> {
    if (!this.endpoint) return { state: "misconfigured", source: this.provider, observedAt: new Date().toISOString(), error: { code: "INVALID_CONFIGURATION", message: "Orca endpoint is not configured", retryable: false } };
    ...
    try {
      ...
      if (!response.ok) return { state: response.status === 429 ? "degraded" : "unavailable", source: this.provider, observedAt: new Date().toISOString(), error: { code: response.status === 429 ? "RATE_LIMITED" : "PROVIDER_UNAVAILABLE", message: "Orca request failed", retryable: response.status >= 429, providerStatus: response.status } };
      const payload = await response.json() as Record<string, unknown>;
      return { state: "available", source: this.provider, observedAt: new Date().toISOString(), data: { provider: "orca", operation: request.operation, payload } };
    } catch (error) {
