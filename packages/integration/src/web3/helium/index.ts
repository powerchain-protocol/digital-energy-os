...
export interface HeliumAdapterRequest { operation: string; payload?: Record<string, unknown>; }
export interface HeliumAdapterResponse { provider: "helium"; operation: string; payload: Record<string, unknown>; }

/** Helium: DePIN hotspot and network data. Never returns synthetic production data. */
export class HeliumAdapter implements IntegrationAdapter<HeliumAdapterRequest, HeliumAdapterResponse> {
  readonly provider = "helium";
  constructor(private readonly endpoint?: string, private readonly credentialReference?: string) {}
  async execute(request: HeliumAdapterRequest, context: IntegrationContext): Promise<IntegrationResult<HeliumAdapterResponse>> {
    if (!this.endpoint) return { state: "misconfigured", source: this.provider, observedAt: new Date().toISOString(), error: { code: "INVALID_CONFIGURATION", message: "Helium endpoint is not configured", retryable: false } };
    ...
    try {
      ...
      if (!response.ok) return { state: response.status === 429 ? "degraded" : "unavailable", source: this.provider, observedAt: new Date().toISOString(), error: { code: response.status === 429 ? "RATE_LIMITED" : "PROVIDER_UNAVAILABLE", message: "Helium request failed", retryable: response.status >= 429, providerStatus: response.status } };
      const payload = await response.json() as Record<string, unknown>;
      return { state: "available", source: this.provider, observedAt: new Date().toISOString(), data: { provider: "helium", operation: request.operation, payload } };
    } catch (error) {
