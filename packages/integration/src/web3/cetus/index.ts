...
export interface CetusAdapterRequest { operation: string; payload?: Record<string, unknown>; }
export interface CetusAdapterResponse { provider: "cetus"; operation: string; payload: Record<string, unknown>; }

/** Cetus: Sui CLMM quote and swap construction. Never returns synthetic production data. */
export class CetusAdapter implements IntegrationAdapter<CetusAdapterRequest, CetusAdapterResponse> {
  readonly provider = "cetus";
  constructor(private readonly endpoint?: string, private readonly credentialReference?: string) {}
  async execute(request: CetusAdapterRequest, context: IntegrationContext): Promise<IntegrationResult<CetusAdapterResponse>> {
    if (!this.endpoint) return { state: "misconfigured", source: this.provider, observedAt: new Date().toISOString(), error: { code: "INVALID_CONFIGURATION", message: "Cetus endpoint is not configured", retryable: false } };
    ...
    try {
      ...
      if (!response.ok) return { state: response.status === 429 ? "degraded" : "unavailable", source: this.provider, observedAt: new Date().toISOString(), error: { code: response.status === 429 ? "RATE_LIMITED" : "PROVIDER_UNAVAILABLE", message: "Cetus request failed", retryable: response.status >= 429, providerStatus: response.status } };
      const payload = await response.json() as Record<string, unknown>;
      return { state: "available", source: this.provider, observedAt: new Date().toISOString(), data: { provider: "cetus", operation: request.operation, payload } };
    } catch (error) {
