...
export interface JupiterAdapterRequest { operation: string; payload?: Record<string, unknown>; }
export interface JupiterAdapterResponse { provider: "jupiter"; operation: string; payload: Record<string, unknown>; }

/** Jupiter: Solana swap quotes and transaction construction. Never returns synthetic production data. */
export class JupiterAdapter implements IntegrationAdapter<JupiterAdapterRequest, JupiterAdapterResponse> {
  ...
  async execute(request: JupiterAdapterRequest, context: IntegrationContext): Promise<IntegrationResult<JupiterAdapterResponse>> {
    if (!this.endpoint) return { state: "misconfigured", source: this.provider, observedAt: new Date().toISOString(), error: { code: "INVALID_CONFIGURATION", message: "Jupiter endpoint is not configured", retryable: false } };
    ...
    try {
      ...
      if (!response.ok) return { state: response.status === 429 ? "degraded" : "unavailable", source: this.provider, observedAt: new Date().toISOString(), error: { code: response.status === 429 ? "RATE_LIMITED" : "PROVIDER_UNAVAILABLE", message: "Jupiter request failed", retryable: response.status >= 429, providerStatus: response.status } };
