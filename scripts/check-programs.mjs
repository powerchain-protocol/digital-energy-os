...
for (const file of modules) {
  const path = `programs/src/${file}`;
  if (!fs.existsSync(path)) throw new Error(`Missing ${path}`);
}

for (const domain of domains) {
  for (const file of ["README.md", "src/lib.rs"]) {
    const path = `programs/${domain}/${file}`;
    if (!fs.existsSync(path)) throw new Error(`Missing ${path}`);
  }
}

const lib = fs.readFileSync("programs/src/lib.rs", "utf8");
for (const module of ["proof_of_energy", "digital_twin", "gridllm", "errors", "events", "invariants"]) {
  if (!lib.includes(`pub mod ${module};`)) throw new Error(`programs/src/lib.rs does not export ${module}`);
}

const invariants = fs.readFileSync("programs/src/invariants.rs", "utf8");
