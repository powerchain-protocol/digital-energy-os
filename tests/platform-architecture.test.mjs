...
test("platform architecture route and API are registered", () => {
  assert.match(read("src/config/routes.ts"), /platform: "\/platform"/);
  assert.ok(fs.existsSync("src/app/platform/page.tsx"));
  assert.ok(fs.existsSync("src/app/api/v1/platform/catalog/route.ts"));
});

test("cloud architecture includes all enterprise layers", () => {
  const catalog = read("src/data/platform/catalog.ts");
...
test("canonical layer folders expose public entry points", () => {
  for (const layer of ["foundation", "cloud", "fabric", "runtime", "domains", "intelligence", "operations", "developer", "experience", "shared"]) {
    assert.ok(fs.existsSync(`src/${layer}/index.ts`), `${layer} index is missing`);
    assert.ok(fs.existsSync(`src/${layer}/README.md`), `${layer} README is missing`);
  }
});
